/** @jsxImportSource @opentui/solid */
import { RGBA } from "@opentui/core"
import { testRender } from "@opentui/solid"
import { expect, test } from "bun:test"
import { createSignal } from "solid-js"
import { PromptInterruptStatus } from "../../../src/component/prompt"

test("armed interrupt status renders visible warning text", async () => {
  const text = RGBA.fromHex("#ffffff")
  const subdued = RGBA.fromHex("#808080")
  const warning = RGBA.fromHex("#fbbf24")
  const flash = RGBA.fromHex("#fde68a")
  const app = await testRender(
    () => (
      <PromptInterruptStatus armed animations={false} text={text} subdued={subdued} warning={warning} flash={flash} />
    ),
    {
      width: 30,
      height: 1,
    },
  )

  try {
    await app.renderOnce()
    const line = app.renderer.currentRenderBuffer.getSpanLines()[0]

    expect(line.spans.map((span) => span.text).join("")).toContain("esc again to interrupt")
    expect(line.spans.filter((span) => span.text.trim()).every((span) => span.fg.equals(warning))).toBeTrue()
  } finally {
    app.renderer.destroy()
  }
})

test("armed interrupt status flashes before settling to warning", async () => {
  const [armed, setArmed] = createSignal(false)
  const warning = RGBA.fromHex("#fbbf24")
  const app = await testRender(
    () => (
      <PromptInterruptStatus
        armed={armed()}
        animations
        text={RGBA.fromHex("#ffffff")}
        subdued={RGBA.fromHex("#808080")}
        warning={warning}
        flash={RGBA.fromHex("#fde68a")}
      />
    ),
    { width: 30, height: 1 },
  )

  try {
    await app.renderOnce()
    setArmed(true)
    await app.renderOnce()
    expect(
      app.renderer.currentRenderBuffer
        .getSpanLines()[0]
        .spans.filter((span) => span.text.trim())
        .every((span) => !span.fg.equals(warning)),
    ).toBeTrue()

    await Bun.sleep(260)
    await app.renderOnce()
    expect(
      app.renderer.currentRenderBuffer
        .getSpanLines()[0]
        .spans.filter((span) => span.text.trim())
        .every((span) => span.fg.equals(warning)),
    ).toBeTrue()
  } finally {
    app.renderer.destroy()
  }
})
